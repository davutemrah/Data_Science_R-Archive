...
So, in bootstrapping the observed data substitutes for what? a population

So, in bootstrapping if the observed data is the population, what would the random samplings correspond to? observations
...
Here's a critical point. In constructing the estimated distribution we sample the observed data WITH replacement. If the original sample is n long and we sampled n times without replacement what would we get? the original sample permuted
...
For the heck of it, compute the expected die roll for a fair die. 3.5
...
B = 10000
nh = length(sh)
resamples <- matrix(sample(sh, nh*B, replace = T), B, nh)
...
```{r}
g = ggplot(as.data.frame(resampledMedians), aes(x=resampledMedians))
